...
Windows용 저지연 캡처 뷰어이자 실험적 Neural Rendering 애플리케이션입니다.
캡처카드의 영상과 오디오를 PC에서 확인하고, 사용자가 별도로 제공한 호환
NVIDIA NR 런타임으로 영상의 외관을 보정할 수 있습니다. 게임 내부에 모드를
주입하는 방식이 아니라 캡처된 영상을 처리합니다.

![DLSS NR Capture에서 캡처카드로 입력된 게임 화면을 표시하는 모습](docs/images/application-screenshot.png)

*사용자가 제공한 실제 애플리케이션 화면. 제목 표시줄에는 Live Gamer Ultra 2.1의
2560×1440, 60FPS, MJPG 입력과 연결된 NR 워커 상태가 표시됩니다.
60FPS는 선택된 입력 모드이며 NR 처리·표시 속도를 보증하지 않습니다.
이 이미지는 사용 예시이며 원본/NR 전후 비교나 화질 검증 자료가 아닙니다.
게임 화면과 관련 상표의 권리는 각 권리자에게 있습니다.*

## 빠른 시작

[릴리즈](https://github.com/lesser-vr/dlss-nr-capture/releases)에서 공개 패키지를 받거나
아래 빌드 절차를 따릅니다. NR은 호환 GPU와 별도 런타임이 필요합니다.
Creative controls에서 NR 해상도 100/75/50%, 색 보존, 하이라이트 보호와
1/2/3패스를 선택할 수 있습니다. 축소 NR은 작은 영상의 변화량을 원본에 합성합니다.
데스크톱/개별 창 캡처, 동영상 플레이어, 프레임 생성, VR 통합 기능은 제공하지 않습니다.
...
이 프로젝트는 NVIDIA의 비공개·유출·수정 런타임을 재배포하지 않습니다. NR
...
flow 전송 경로와 원래 오류 코드를 확인할 수 있습니다. 앱·워커·어댑터는 같은 빌드로
