...
public:
    ...
    bool initialize(ID3D11Device* device, const D3D11_TEXTURE2D_DESC& description) override {
        if (GetFileAttributesW(path_.c_str()) == INVALID_FILE_ATTRIBUTES) { error_ = 0; return false; }
        module_ = LoadLibraryExW(path_.c_str(), nullptr,
            LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR | LOAD_LIBRARY_SEARCH_SYSTEM32);
        if (!module_) { error_ = 1; return false; }
        auto get_api = reinterpret_cast<NrAdapterGetApi>(GetProcAddress(module_, "DlssNrAdapterGetApi"));
        if (!get_api) { error_ = 2; return false; }
        ...
        if (!api_ || api_->byte_size < sizeof(NrAdapterApi) ||
            ...
            !api_->process || !api_->shutdown || !api_->display_name) {
            error_ = 3; return false;
        }
        if (!api_->initialize(device, &description)) { error_ = 4; return false; }
        initialized_ = true;
        error_ = 0;
        return true;
    }
    bool process(ID3D11DeviceContext* context, ID3D11Texture2D* texture,
        ...
        return initialized_ && api_->process(context, texture, &payload);
    }
